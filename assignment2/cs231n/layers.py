import numpy as np
#some of the following codes come from github user ncchen55414,umbanhowar and Standford cs231n
#some ideas come from multiple sources, like stackoverflo, learning notes of cs231n....
...
def conv_forward_naive(x, w, b, conv_param):
	...
	for i in range(N):		 # ith image	
		for j in range(F):	 # jth filter		
			for p in range(int(H_new)):			
				for k in range(int(W_new)):
					length1 = p*stride #simplify the np.sum
					length2 = k*stride				 
					out[i, j, p, k] = np.sum(x_pad[i, :, length1:HH+length1, length2:WW+length2] * w[j]) + b[j]
...
def conv_backward_naive(dout, cache):
	...
	for i in range(N):			 # ith image		
			for j in range(F):	 # fth filter				
					for p in range(int(H_new)):						
							for k in range(int(W_new)):
									length1 = p*stride #simplify the np.sum
									length2 = k*stride									 
									db[j] += dout[i, j, p, k]								
									dw[j] += x_padded[i, :, length1:HH+length1, length2:WW+length2] * dout[i, j, p, k]								
									dx_padded[i, :, length1:HH+length1, length2:WW+length2] += w[j] * dout[i, j, p, k]
			...
			for j in range(C):				
					for k in range(int(H_new)):						
							for l in range(int(W_new)):								
									x_mask = x[i, j, k*stride:HH+k*stride, l*stride:WW+l*stride] 
									out[i, j, k, l] = np.max(x_mask)
